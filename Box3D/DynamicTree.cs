...
namespace Box3D
{
    ...
    public sealed unsafe class DynamicTree : IDisposable
    {
        ...
        public DynamicTree(int proxyCapacity = 16)
        {
            ...
            *_tree = Ffi.b3DynamicTree_Create(proxyCapacity);
        }
        ...
        private void DisposeNative()
        {
            ...
            Ffi.b3DynamicTree_Destroy(_tree);
        ...
        public int CreateProxy(B3Aabb aabb, ulong userData, ulong categoryBits = 1)
            => Ffi.b3DynamicTree_CreateProxy(_tree, aabb, categoryBits, userData);

        public void DestroyProxy(int proxyId) => Ffi.b3DynamicTree_DestroyProxy(_tree, proxyId);

        /// <summary>Re-fits a proxy to a new AABB (remove + reinsert). Use as objects move.</summary>
        public void MoveProxy(int proxyId, B3Aabb aabb) => Ffi.b3DynamicTree_MoveProxy(_tree, proxyId, aabb);

        /// <summary>Grows a proxy's fat AABB (and ancestors) — cheaper than Move when the box only expands.</summary>
        public void EnlargeProxy(int proxyId, B3Aabb aabb) => Ffi.b3DynamicTree_EnlargeProxy(_tree, proxyId, aabb);

        public void SetCategoryBits(int proxyId, ulong categoryBits) => Ffi.b3DynamicTree_SetCategoryBits(_tree, proxyId, categoryBits);

        public ulong GetCategoryBits(int proxyId) => Ffi.b3DynamicTree_GetCategoryBits(_tree, proxyId);

        // ---- introspection ----

        public int ProxyCount => Ffi.b3DynamicTree_GetProxyCount(_tree);

        public int Height => Ffi.b3DynamicTree_GetHeight(_tree);

        public float AreaRatio => Ffi.b3DynamicTree_GetAreaRatio(_tree);

        public B3Aabb RootBounds => Ffi.b3DynamicTree_GetRootBounds(_tree);

        public int ByteCount => Ffi.b3DynamicTree_GetByteCount(_tree);
        ...
        public int Rebuild(bool fullBuild = false) => Ffi.b3DynamicTree_Rebuild(_tree, fullBuild);

        // ---- validation (debug / tests) ----

        /// <summary>Asserts internal invariants (box3d asserts on failure). For tests/debugging.</summary>
        public void Validate() => Ffi.b3DynamicTree_Validate(_tree);

        public void ValidateNoEnlarged() => Ffi.b3DynamicTree_ValidateNoEnlarged(_tree);
        ...
        public void Save(string fileName)
        {
            ...
            fixed (byte* pp = p)
            {
                Ffi.b3DynamicTree_Save(_tree, (sbyte*)pp);
            }
        }
        ...
        public static DynamicTree Load(string fileName, float scale = 1f)
        {
            ...
            fixed (byte* pp = p)
            {
                Ffi.b3DynamicTree_Destroy(tree._tree); // free the empty pool the ctor made
                *tree._tree = Ffi.b3DynamicTree_Load((sbyte*)pp, scale);
            }
            return tree;
        }
            ...
            ulong maskBits = ulong.MaxValue, bool requireAllBits = false)
        {
            fixed (DynamicTreeHit* buffer = results)
            {
                ...
                b3TreeStats s = Ffi.b3DynamicTree_Query(_tree, aabb, maskBits, requireAllBits, QueryPtr, &ctx);
                stats = new TreeStats(s.nodeVisits, s.leafVisits);
                return ctx.Count;
            }
        }
            ...
            float maxFraction = 1f, ulong maskBits = ulong.MaxValue, bool requireAllBits = false)
        {
            ...
            {
                var ctx = new CollectorContext { Buffer = buffer, Capacity = results.Length };
                b3TreeStats s = Ffi.b3DynamicTree_RayCast(_tree, &input, maskBits, requireAllBits, RayPtr, &ctx);
                stats = new TreeStats(s.nodeVisits, s.leafVisits);
                return ctx.Count;
            }
        }
            ...
            float maxFraction = 1f, ulong maskBits = ulong.MaxValue, bool requireAllBits = false)
        {
            ...
            {
                var ctx = new CollectorContext { Buffer = buffer, Capacity = results.Length };
                b3TreeStats s = Ffi.b3DynamicTree_BoxCast(_tree, &input, maskBits, requireAllBits, BoxPtr, &ctx);
                stats = new TreeStats(s.nodeVisits, s.leafVisits);
                return ctx.Count;
            }
        }
